...
public class UpdateRoomController extends HttpServlet {
    private static final String ERROR = "UpdateController";
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (st) {
                dao.activeRoom(roomID);
            }
            RoomDTO room = new RoomDTO(roomID, name, description, typeID, img, st, price, max);
            boolean rs =dao.updateRoom(room);
            if (rs){
                url = SUCCESS;
                List<RoomDTO> listRoom = dao.getListRoom();
                HttpSession session = request.getSession();
                session.setAttribute("FULL_LIST_ROOM", listRoom);
            }
        } catch (Exception e) {
        } finally {
